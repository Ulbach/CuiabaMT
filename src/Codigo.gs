...
function loginSeguranca(codigo) {
  ...
  if (!codigoInformado) return { ok: false, message: "Informe o código de acesso." };

  const cadastro = buscarSegurancaPorCodigo(codigoInformado);
  if (!cadastro) return { ok: false, message: "Código inválido." };
  ...
  return {
    ...
    message: "Olá, " + cadastro.seguranca + "!"
  };
}

function validarSessaoSeguranca(token) {
  const sessao = lerSessaoSeguranca(token);
  if (!sessao) return { ok: false, autenticado: false, message: "Sessão inválida ou expirada." };
...
function salvarVisitante(jsonData) {
  ...
  if (token) {
    const sessao = lerSessaoSeguranca(token);
    if (!sessao) return { ok: false, message: "Sessão inválida ou expirada." };
  ...
  if (p.tipo === "SAIDA") {
    sh.getRange(p.linha, 5).setValue("CONCLUÍDO");
...
function registrarSaidaFrota(p) {
  ...
  for (let i = dados.length - 1; i >= 0; i--) {
    ...
    if (mesmoVeiculo(r[0], p.veiculo) && statusEmTransito(r[6])) {
      return { ok: false, message: "Veículo já está em trânsito." };
    }
  }
...
function registrarEntradaFrota(p) {
  ...
  for (let i = data.length - 1; i >= 0; i--) {
    if (mesmoVeiculo(data[i][0], p.veiculo) && statusEmTransito(data[i][6])) {
      ...
      if (kmRet < kmSai) {
        return { ok: false, message: "KM de retorno inferior ao KM de saída." };
      }

      sh.getRange(linha, 3).setValue(sessao.seguranca);
      sh.getRange(linha, 7).setValue("CONCLUÍDO");
  ...
  return { ok: false, message: "Veículo não encontrado em trânsito." };
}
